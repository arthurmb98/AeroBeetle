...
<body>
 ...
 <div id="main" class="container-fluid" style="margin-top: 50px">
 	...
 	<div id="top" class="row">
		...
		<div class="col-sm-3">
			<a href="../curso_crud/add.php" class="btn btn-primary pull-right h2">Novo Curso</a>
		</div>
	</div> <!-- /#top -->
		...
		<table class="table table-striped" cellspacing="0" cellpadding="0">
			...
			<?php 
                  ...
                  if(isset($_GET['idButton'])){
                    DAOAluno::getInstance()->matricular($_GET['idButton']);
                  }else{
                    if(isset($_GET['idCurso']) && isset($_GET['idAluno'])){
                      
                        try{
                            $sql = "INSERT INTO alunocurso(idAluno, idCurso, sala) VALUES (?,?,?)";
                            $stmt = Conexao::getInstance()->prepare($sql);
                            $stmt->bindParam(1, $_GET['idAluno']); 
                            $stmt->bindParam(2, $_GET['idCurso']); 
                            $stmt->bindParam(3, $_GET['Sala']); 
                            $stmt->execute();				
                            Log::getInstance()->inserir(LOCAL_DA,"INSERT na tabela `alunocurso` executada...");
                            $okay = true;
                            echo"<script>
                            alert('Aluno Matriculado com sucesso!');
                            window.location.href='../aluno_crud/';
                            </script>";	
