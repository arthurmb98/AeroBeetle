	...
	<div class="table-responsive col-md-12">
      ...
      <?php 
       ...
       if(isset($_GET['idButton'])){
         DAOAluno::getInstance()->alunoprofessor($_GET['idButton']);
       }else{
         if(isset($_GET['idProfessor']) && isset($_GET['idAluno'])){
           
             try{
                 $sql = "INSERT INTO alunoprofessor(idAluno, idProfessor) VALUES (?,?)";
                 $stmt = Conexao::getInstance()->prepare($sql);
                 $stmt->bindParam(1, $_GET['idAluno']); 
                 $stmt->bindParam(2, $_GET['idProfessor']); 
                 $stmt->execute();				
                 Log::getInstance()->inserir(LOCAL_DA,"INSERT na tabela `alunoprofessro` executada...");
                 $okay = true;
                 echo"<script>
